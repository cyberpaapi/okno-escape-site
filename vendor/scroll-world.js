...
function mountScrollWorld(container, config) {
  ...
  // Source and input detection are separate: a tall 1440x2560 preview receives the
  // native 9:16 chain, while only small/coarse devices use lighter seeking behaviour.
  const coarse = window.matchMedia('(hover: none) and (pointer: coarse)').matches;
  const smallMQ = window.matchMedia('(max-width: 860px)');
  const portraitMQ = window.matchMedia('(orientation: portrait)');
  const usePortrait = () => smallMQ.matches || portraitMQ.matches;
  ...
  SEGMENTS.forEach(s => {
    ...
    const poster = (usePortrait() && s.stillM) ? s.stillM : s.still;
  ...
  function loadClip(s) {
    ...
    const url = (usePortrait() && s.clipM) ? s.clipM : s.clip;
    const cached = window.__oknoAssetCache?.get(url);
    const blobPromise = cached?.blob
      ? Promise.resolve(cached.blob)
    ...
    blobPromise
      .then(blob => {
        ...
        const objectUrl = cached?.objectUrl || URL.createObjectURL(blob);
        window.__oknoAssetCache?.set(url, { blob, objectUrl });
        v.src = objectUrl;
